...
public class BTree<Key extends Comparable<Key>, Value> {
    ...
    public Value get(Key key) {
        if(isEmpty()) throw new IllegalArgumentException("argument to get() is null");
    ...
    private Node insert(Node x, Key key, Value value, int ht) {
        int j;
        ...
        if(ht == 0) {
            for(j=0; j<x.m; j++)
                if(less(key, x.children[j].key)) break;
        } else {
            for(j=0; j<x.m; j++) {
                if(j+1 == x.m || less(key, x.children[j+1].key)) {
                    Node u = insert(x.children[j++].next, key, value, ht-1);
                    if(u==null) return null;
                    t.key = x.children[0].key;
                    t.next = x;
                    break;
                }
            }
